...
[0] = LAYOUT_65_ansi(
  ...
  KC_TAB,  KC_Q,    KC_W,   KC_E,   KC_R,   KC_T,   KC_Z,   DUAL_U,   KC_I,    DUAL_O,    KC_P,    KC_LBRC, KC_RBRC, KC_BSLS, KC_DEL, \
  KC_CAPS, DUAL_A,    KC_S,   KC_D,   KC_F,   KC_G,   KC_H,   KC_J,   KC_K,    KC_L,    KC_SCLN, KC_QUOT,          KC_ENT, KC_PGUP, \
...
static uint8_t tap_hold_mod_state;
void send_custom_tap_hold(keyrecord_t *record, char* tap, char* hold) {
    if(record->event.pressed) {
        tap_hold_timer = timer_read();
        tap_hold_mod_state = get_mods();
        send_string(tap); // Write out tap character right away
      } else {
        if (timer_elapsed(tap_hold_timer) > TAPPING_TERM) {
            uint8_t mod_state = get_mods();
            set_mods(tap_hold_mod_state); // Restore to initially pressed mods to behave more reliably
            send_string(SS_TAP(X_BSPC)); // Delete initially tapped character
            send_string(hold); // ...and replace it with the hold character
            set_mods(mod_state);
        }
      }
}
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  case ISO_MINS:
      ...
      break;
  case DUAL_A:
      send_custom_tap_hold(
        record,
        SS_TAP(X_A), // a (tap)
        SS_RALT(SS_TAP(X_A)) // ä (hold)
      );
      break;
  case DUAL_O:
      send_custom_tap_hold(
